...
interface Product { id: number; name: string; description: string; price: number; category: Category; imageUrl?: string; imageEmoji?: string; stock: number; available: boolean; }
...
export default function ProductsAdminPage() {
  ...
  const [form, setForm] = useState({ name: "", description: "", price: "", category: "suplementos" as Category, imageUrl: "", stock: "0" });
  ...
  const startCreate = () => { setEditing(null); setShowForm(true); setForm({ name: "", description: "", price: "", category: "suplementos", imageUrl: "", stock: "0" }); setError(""); };
  const startEdit = (product: Product) => { setEditing(product); setShowForm(true); setForm({ name: product.name, description: product.description, price: String(product.price), category: product.category, imageUrl: product.imageUrl || "", stock: String(product.stock) }); setError(""); };

  const save = async (event: FormEvent) => {
    event.preventDefault(); setSaving(true); setError("");
    const response = await authFetch(editing ? `/products/${editing.id}` : "/products", { method: editing ? "PUT" : "POST", body: JSON.stringify({ ...form, price: Number(form.price), stock: Number(form.stock) }) });
